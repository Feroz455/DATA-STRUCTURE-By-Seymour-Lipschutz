...
void print_predeccessor(list<int> A, list<int> X[], int temp)
{
    list<int>::iterator ptr = A.begin();
    while (ptr != A.end())
    {
...
void print_successors(list<int> X[], int i)
{
    list<int>::iterator ptr = X[i].begin();
    while (ptr != X[i].end())
    {
        cout << char((*ptr++) + 65) << " -> ";
    }
...
int main()
{
    ...
    cout << n << "'s "
         << "SUCCSSEOR is -> ";
    print_successors(X, SRC);
    cout << "\n"
         << n << "'s "
         << "PRECCSSEOR is -> ";
    print_predeccessor(A, X, SRC);
}
/*
